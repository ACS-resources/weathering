# StandardMap.cs（Java 迁移解构文档）

- 源文件：`Assets/Game/StandardMap.cs`
- 命名空间：`Weathering`
- 代码行数：`498`
...
- `class CameraX`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class CameraY`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class CharacterX`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class CharacterY`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class StandardMap`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `struct TemporatureConfig`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `struct MoistureConfig`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `struct AltitudeConfig`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。

## 3. 函数/属性接口梳理

### 3.1 方法签名

- `public EnterParentMap()`
- `public EnterChildMap(Vector2Int pos)`
- `public GetSpriteKeyBedrock(Vector2Int pos)`
- `public GetSpriteKeyGrass(Vector2Int pos)`
- `public GetSpriteKeyWater(Vector2Int pos)`
- `public GetSpriteKeyTree(Vector2Int pos)`
- `public GetSpriteKeyHill(Vector2Int pos)`
- `public OnConstruct()`
- `protected SetCharacterPos(Vector2Int characterPosition, bool sync=false)`
- `protected SetCameraPos(Vector2 cameraPos)`
- `protected SetClearColor(Color clearColor)`
- `public OnEnable()`
- `public OnDisable()`
...
- `public CanUpdateAt(Type type, Vector2Int pos)`
- `public CanUpdateAt(Type type, int i, int j)`
- `public Get(int i, int j)`
- `public Get(Vector2Int pos)`
- `public UpdateAt(Type type, ITile oldTile)`
- `private UpdateAt(Type type, int i, int j, ITile oldTile)`
- `public Validate(Vector2Int pos)`
- `private Validate(ref int i, ref int j)`
- `public SetTile(Vector2Int pos, ITileDefinition tile, bool inConstruction = false)`
- `public Create()`
- `public Create()`
- `public Create()`
- `private GenerateNoise()`
- `private GenerateResources()`
- `private GenerateAltitude()`
- `private GenerateMoisture()`
- `private GenerateTemporature()`
- `public OnTapTile(ITile tile)`
- `public Delete()`
- `public GetTileFast(int i, int j)`
- `public AfterConstructMapBody()`
...
- `protected GetAltitudeConfig`
- `protected GetMoistureConfig`
- `protected GetTemporatureConfig`
- `public TemporatureConfig`
- `public MoistureConfig`
- `public AltitudeConfig`
- `public Altitudes`
- `public AltitudeTypes`
- `public Moistures`
- `public MoistureTypes`
- `public Temporatures`
- `public TemporatureTypes`
- `public ResourceTypes`
- `protected RandomSeed`
- `private AutoInc`
- `public MapKey`
- `public ParentPositionBuffer`
