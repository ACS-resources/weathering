# HashUtility.cs（Java 迁移解构文档）

- 源文件：`Assets/Scripts/Core/RandomnessGenerator/HashUtility.cs`
- 命名空间：`Weathering`
- 代码行数：`248`
...
- `class HashUtility`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
...
- `public Hash(uint a)`
- `public Hashed(ref uint x)`
- `public AddSalt(uint a, uint salt)`
- `public ToFloat(uint hash)`
- `public RandomVec2(int i, int j, int width, int height)`
- `public RandomVec2Simple(int i, int j, int width, int height, int offset = 0)`
- `public Hash(int i, int j, int width, int height, int offset = 0)`
- `public Hash(Vector2Int pos, Vector2Int size)`
- `public SimpleValueNoise(float x)`
- `public SimpleValueNoise(double x)`
- `public PerlinNoise(float x, float y, int width, int height, int layer = 0)`
- `public Hash0(uint a)`
- `public Hash1(uint a)`
- `public Hash2(uint a)`
- `public Hash3(uint a)`
