# AttributesPreprocessor.cs（Java 迁移解构文档）

- 源文件：`Assets/Scripts/Core/Concept/AttributesPreprocessor.cs`
- 命名空间：`Weathering`
- 代码行数：`237`
...
- `class DependAttribute`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class ConceptAttribute`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class AttributesPreprocessor`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。

## 3. 函数/属性接口梳理

### 3.1 方法签名

- `private GenerateLocalizationFile()`
- `private Awake()`
- `private Depend(Type type1, Type type2)`
- `public Compare(Type x, Type y)`
