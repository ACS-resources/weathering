# IInventory.cs（Java 迁移解构文档）

- 源文件：`Assets/Scripts/Core/IInventory.cs`
- 命名空间：`Weathering`
- 代码行数：`495`
...
- `struct InventoryItemData`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IInventory`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IInventoryDefinition`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Inventory`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。

## 3. 函数/属性接口梳理

### 3.1 方法签名

- `public SetVal(long val)`
- `public AddVal(long val)`
- `public SetQuantity(long value)`
- `public CanRemove(Type type)`
- `public CanRemove(ValueTuple<Type, long> pair)`
- `public Clear(Type type)`
- `public Add(Type type, long val)`
- `public Add(ValueTuple<Type, long> pair)`
- `public CanAdd(Type type)`
- `public CanAdd(ValueTuple<Type, long> pair)`
- `public Remove(Type type, long val)`
- `public Remove(ValueTuple<Type, long> pair)`
- `public GetWithTag(Type type)`
- `public AddFrom(Type type, IInventory other, long max = long.MaxValue)`
- `public AddFrom(Type type, IValue other, long max = long.MaxValue)`
- `public RemoveWithTag(Type type, long val, Dictionary<Type, InventoryItemData> canRemove = null, Dictionary<Type, InventoryItemData> removed = null)`
- `public RemoveWithTag(ValueTuple<Type, long> pair)`
- `public CanRemoveWithTag(Type type, Dictionary<Type, InventoryItemData> canRemoveAccumulated = null, long val = long.MaxValue)`
- `public CanRemoveWithTag(ValueTuple<Type, long> pair)`
- `public CanAddWithTag(Dictionary<Type, InventoryItemData> other, long val)`
- `public CanAddEverything(Dictionary<Type, InventoryItemData> other)`
- `public AddEverything(Dictionary<Type, InventoryItemData> everything)`
- `public AddEverythingFrom(IInventory other)`
- `public Clear()`
- `public ToData(IInventory inventory)`
- `public FromData(DataPersistence.InventoryData data)`
- `public GetOne()`
