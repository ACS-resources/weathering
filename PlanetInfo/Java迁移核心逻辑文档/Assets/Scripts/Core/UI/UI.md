...
- 源文件：`Assets/Scripts/Core/UI/UI.cs`
- 命名空间：`Weathering`
- 代码行数：`618`
...
- `interface IUI`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class UI`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
...
- `private Awake()`
- `public SetBackgroundTransparency(float a)`
- `private SetAlpha(Color c, float a)`
- `public OnTapTitle()`
- `public SetExitIndicatorVisible(bool val)`
- `private DestroyChildren()`
- `private CreateTransparency(int scale)`
- `private CreateBarImage(string content = null, Func<string> dynamicContent = null,
            Action onTap=null,
            int scale = 1, int leftPadding = 64,
            bool useSeparator = false)`
- `private CreateText(string content)`
- `private CreateButton(IUIBackgroundType background, string label = null, string icon = null, Action onTap = null,
                bool interactable=true, Func<bool> canTap = null, Func<string> dynamicContent = null)`
- `private CreateOneLineDynamicText(Func<string> dynamicText)`
- `private CreateOneLineStaticText(string content)`
- `private CreateProgressBar()`
- `private CreateSlider(Func<float, string> dynamicSlider, float initialValue=0)`
- `private CreateValueProgress(IValue value, string barTitle = "")`
- `private CreateTimeProgress(IValue value, string barTitle = "")`
- `private CreateDelProgress(IValue value, string barTitle = "")`
- `public PlaySound()`
- `private Update()`
- `private CalcUpdateValueProgress(IValue value)`
- `private UpdateValueProgress(ProgressBar key, IValue value, string title)`
- `private CalcUpdateTimeProgress(IValue value)`
- `private UpdateTimeProgress(ProgressBar key, IValue value, string title)`
- `private CalcUpdateDelProgress(IValue value)`
- `private UpdateDelProgress(ProgressBar key, IValue value, string title)`
- `public GetItems()`
- `public ShowItems(string title, List<IUIItem> IUIItems)`
- `public Error(Exception e)`
- `public ShowItems(string title, params IUIItem[] uiitems)`
