# MapView.cs（Java 迁移解构文档）

- 源文件：`Assets/Scripts/Core/Map/MapView.cs`
- 命名空间：`Weathering`
- 代码行数：`1502`
...
- `interface IPassable`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IStepOn`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IWeatherDefinition`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IMapView`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IIgnoreTool`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface IHasFrameAnimationOnSpriteKey`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class MapView`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class TappingSensitivity`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
...
- `private Start()`
- `private Update()`
- `public SyncCharacterPosition()`
- `private UpdateCameraWidthArrowKey()`
- `private UpdateCameraWithTapping()`
- `private UpdateCharacterPositionWithTapping()`
- `private UpdateCharacterPositionWithArrowKey()`
- `private TryTriggerOnStepEvent()`
- `private CorrectCharacterPositionAndLight()`
- `private GetRealPositionOfCharacter()`
- `private CameraFollowsCharacter()`
- `private CharacterLightFollowCharacter()`
- `private CorrectCameraPosition()`
- `private ThrowSpriteNotFoundException(string spriteName, ITile tile, string info)`
- `private UpdateMap()`
- `private UpdateMapAnimation()`
- `private UpdateWeather()`
- `private UpdateIndicator(Vector2Int pos)`
- `private ScreenToWorldPoint(Vector2 mousePosition)`
- `private UpdateInput()`
- `private MathVector2Floor(Vector2 vec)`
- `private OnTap(Vector2Int pos)`

### 3.2 属性签名

- `public IPassable`
- `public IStepOn`
- `public IWeatherDefinition`
- `public IMapView`
- `public IIgnoreTool`
- `public IHasFrameAnimationOnSpriteKey`
- `public Ins`
- `public TheOnlyActiveMap`
- `public CameraSize`
- `public TargetTexture`
- `public OnlyThingScale`
- `public CameraPosition`
- `public CharacterPosition`
- `public ClearColor`
- `public TappingSensitivity`
- `public TappingSensitivityFactor`
- `public AnimationIndex`
- `public CameraWidthHalf`
- `public CameraHeightHalf`
- `public EnableLight`
