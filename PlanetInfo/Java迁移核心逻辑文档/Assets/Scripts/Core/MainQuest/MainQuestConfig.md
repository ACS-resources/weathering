# MainQuestConfig.cs（Java 迁移解构文档）

- 源文件：`Assets/Scripts/Core/MainQuest/MainQuestConfig.cs`
- 命名空间：`Weathering`
- 代码行数：`535`
...
- `class Quest_CongratulationsQuestAllCompleted`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_LandRocket`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectFood_Initial`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ConstructBerryBushAndWareHouse_Initial`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectFood_Hunting`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_HavePopulation_Settlement`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectFood_Agriculture`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_HavePopulation_PopulationGrowth`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectWood_Woodcutting`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceWoodProduct_WoodProcessing`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectStone_Stonecutting`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceStoneProduct_StoneProcessing`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ConstructBridge`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceToolPrimitive`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_HavePopulation_ResidenceConstruction`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceWheelPrimitive`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_GainGoldCoinThroughMarket`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectMetalOre_Mining`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceMetal_Smelting`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceMetalProduct_Casting`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceMachinePrimitive`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_CollectCoal`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceSteel`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceConcrete`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceBuildingPrefabrication`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceElectricity`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceAluminum`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceLPG`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProducePlastic`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class Quest_ProduceLightMaterial`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class MainQuestConfig`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
...
- `private Awake()`
- `public GetIndex(Type quest)`
- `private FAQ(string question)`
- `private CreateOnTapQuest()`
- `public QuestConfigNotProvidedThrowException(Type type)`

### 3.2 属性签名

- `public Quest_CongratulationsQuestAllCompleted`
- `public Quest_LandRocket`
- `public Quest_CollectFood_Initial`
- `public Quest_ConstructBerryBushAndWareHouse_Initial`
- `public Quest_CollectFood_Hunting`
- `public Quest_HavePopulation_Settlement`
- `public Quest_CollectFood_Agriculture`
- `public Quest_HavePopulation_PopulationGrowth`
- `public Quest_CollectWood_Woodcutting`
- `public Quest_ProduceWoodProduct_WoodProcessing`
- `public Quest_CollectStone_Stonecutting`
- `public Quest_ProduceStoneProduct_StoneProcessing`
- `public Quest_ConstructBridge`
- `public Quest_ProduceToolPrimitive`
- `public Quest_HavePopulation_ResidenceConstruction`
- `public Quest_ProduceWheelPrimitive`
- `public Quest_GainGoldCoinThroughMarket`
- `public Quest_CollectMetalOre_Mining`
- `public Quest_ProduceMetal_Smelting`
- `public Quest_ProduceMetalProduct_Casting`
- `public Quest_ProduceMachinePrimitive`
- `public Quest_CollectCoal`
- `public Quest_ProduceSteel`
- `public Quest_ProduceConcrete`
- `public Quest_ProduceBuildingPrefabrication`
- `public Quest_ProduceElectricity`
- `public Quest_ProduceAluminum`
- `public Quest_ProduceLPG`
- `public Quest_ProducePlastic`
- `public Quest_ProduceLightMaterial`
- `public Ins`
