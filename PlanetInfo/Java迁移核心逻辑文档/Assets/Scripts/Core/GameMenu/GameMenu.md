# GameMenu.cs（Java 迁移解构文档）

- 源文件：`Assets/Scripts/Core/GameMenu/GameMenu.cs`
- 命名空间：`Weathering`
- 代码行数：`889`
...
- `class GameMenuExitGame`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuSaveGame`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuResetGame`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameSettings`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameLanguage`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuLanguageLabel`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuInspectInventory`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuGotoMainMap`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuResetGameConfirmation`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenuLabel`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class UserInterfaceBackgroundTransparency`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class UtilityButtonsOnTheLeft`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class LogisticsAnimationIsLinear`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class InversedMovement`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class EnableLight`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class EnableWeather`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class ToneMapping`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `interface ITileDescription`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameMenu`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `enum ShortcutMode`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class UsePixelFont`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
...
- `private Awake()`
- `public SetVisible(bool visible)`
- `public SetTileDescriptionForStandalong(string text)`
- `public PushNotification(string notice)`
- `private InitializeNotification()`
- `private Update()`
- `private SetA(Color c, float a)`
- `private Start()`
- `public OnConstruct()`
- `public RestoreDefaultSettings()`
- `public SynchronizeSettings()`
- `private SyncSound()`
- `public SyncEnableLight()`
- `public SyncEnableWeather()`
- `public SyncToneMapping()`
- `private SyncCameraSensitivity()`
- `private SyncUserInterfaceBackgroundTransparency()`
- `private SyncInversedMovement()`
- `private SyncUtilityButtonPosition()`
- `private SyncLogisticsAnimation()`
- `public SyncHammer()`
- `public OnTapConstructDestruct()`
- `public SyncMagnet()`
- `public OnTapLinkUnlink()`
- `public OnTapRunStop()`
- `private SyncButtonsOutlines()`
- `public OnTapQuest()`
- `public OnTapPlayerInventory()`
- `public OnTapInventoryOfResource()`
- `public OnTapInventoryOfSupply()`
- `public OnTapSettings()`
- `private OnTapSaveGameButton()`
- `public SetFont(bool pixel)`
- `public ChangeFont()`
- `public SynchronizeFont()`
- `private OpenConsole()`
- `public OpenGameSettingMenu()`
- `private ToneMappingPage()`
- `private CreateToneMappingButton(long current, long other)`
- `private CalcToneMappingName(long val)`

### 3.2 属性签名

- `public GameMenuExitGame`
- `public GameMenuSaveGame`
- `public GameMenuResetGame`
- `public GameSettings`
- `public GameLanguage`
- `public GameMenuLanguageLabel`
- `public GameMenuInspectInventory`
- `public GameMenuGotoMainMap`
- `public GameMenuResetGameConfirmation`
- `public GameMenuLabel`
- `public UserInterfaceBackgroundTransparency`
- `public UtilityButtonsOnTheLeft`
- `public LogisticsAnimationIsLinear`
- `public InversedMovement`
- `public EnableLight`
- `public EnableWeather`
- `public ToneMapping`
- `public ITileDescription`
- `public Entry`
- `public Ins`
- `public IsInEditor`
- `public IsInStandalone`
- `public IsInMobile`
- `public LightEnabled`
- `public WeatherEnabled`
- `public UseInversedMovement`
- `public IsLinear`
- `public ShortcutMode`
- `public CurrentShortcutMode`
- `private UsePixelFont`
