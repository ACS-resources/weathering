# GameEntry.cs（Java 迁移解构文档）

- 源文件：`Assets/GameEntry/GameEntry.cs`
- 命名空间：`Weathering`
- 代码行数：`437`
...
- `class GameAutoSaveInterval`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
- `class GameEntry`：建议在 Java 中映射为同名（或语义等价）类型，并保留可见性与职责边界。
...
- `private Awake()`
- `private Start()`
- `private GetPosInParentMap(string mapKey)`
- `private SliceParentMapKeyIndex(string mapKey)`
- `private ConstructParentMapKey(Type parentType, string selfIndex)`
- `public EnterParentMap(Type parentType, IMapDefinition mapDefinition)`
- `private SliceSelfMapKeyIndex(string mapKey)`
- `public EnterChildMap(Type childType, IMapDefinition mapDefinition, Vector2Int pos)`
- `private ConstructMapKeyIndexWithPosition(string mapKey, Vector2Int pos)`
- `public ChildMapKeyHashCode(IMap map, Vector2Int pos)`
- `public SelfMapKeyHashCode(IMap map)`
- `public EnterMap(string mapKey, bool enterChildMap = false)`
- `private EnterMap(Type selfType, string selfIndex, string selfKeyVertify = null, bool enterChildMap = false)`
- `public DeleteMap(IMapDefinition map, bool enterSelf=false)`
- `public GetParentTile(Type parentType, IMapDefinition mapDefinition)`
- `private ConstructMapBody(IMapDefinition map)`
- `private Update()`
- `public TrySaveGame()`
- `public SaveGame()`
- `public DeleteGameSave()`
- `public ExitGame()`
- `public ExitGameUnsaved()`
- `private ExitGameInternal(bool save = true)`

### 3.2 属性签名

- `public GameAutoSaveInterval`
- `public Ins`
- `public FrameCount`
