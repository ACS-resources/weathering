...
## Primary planet runtime renderer (non-Unity)

A minimal runtime viewer is available under `com.weathering.mapped`, with startup values copied from original `MapOfPlanet` behavior:
- primary map key: `Weathering.MapOfPlanet#=1,4=14,93=24,31`
- landing position: `(4,83)`
- random seed: `5`

Run:

```bash
java -cp out com.weathering.mapped.PrimaryPlanetCli
```

Behavior:
- Renders the primary planet immediately at start (raw orientation, no flip/center QoL transforms).
- Uses `w/a/s/d` to move camera and `q` to quit.
- Shows terrain and ore characters directly in terminal output.
